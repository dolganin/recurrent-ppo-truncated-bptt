...
class PPOTrainer:
    def __init__(self, config:dict, run_id:str="run", device:torch.device=torch.device("cpu")) -> None:
        ...
        self.video_dir = f"./logs/{self.run_id}/eval_gifs"
        ...
        self.model = ActorCriticModel(self.config, device, self.observation_space, self.action_space_shape,).to(self.device)
        self.model.train()
    ...
    def _record_eval_episode(self, step: int) -> None:
        ...
        # временно подменяем число воркеров на 1
        ...
        while not done and steps < max_steps:
            with torch.no_grad():
                pol, _, rc = self.model(torch.tensor(obs).unsqueeze(0), rc, self.device)
                act = np.array([p.probs.argmax(-1).item() for p in pol])
            obs, _, done, _ = env.step(act)
            frame = env.render()
            frame = frame[::2, ::2]  # даунскейлим
            frames.append(frame)
            steps += 1
    ...
    def _train_mini_batch(self, samples:dict, learning_rate:float, clip_range:float, beta:float) -> list:
        ...
        # Клип градиентов и шаг оптимизатора
    ...
    def _save_model(self) -> None:
        """Saves the model in both .nn (pickle) and .pth (torch) formats."""
